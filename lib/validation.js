...
export function resolveTaskUrl(taskId, env = process.env) {
  const text = requireText(taskId, 'taskId');
  const template = String(env.BONC_OA_TODO_DETAIL_URL_TEMPLATE || '').trim();
  if (template) return template.replace(/\{taskId\}/g, encodeURIComponent(text));
...
export function normalizeApprovalArgs(kwargs, action) {
  const rawTaskId = kwargs.taskId ?? kwargs.taskIds;
  let taskIds;
  if (Array.isArray(rawTaskId)) {
    taskIds = rawTaskId.map((v) => requireText(v, 'taskId')).filter(Boolean);
  } else if (typeof rawTaskId === 'string') {
    taskIds = rawTaskId.split(',').map((v) => v.trim()).filter(Boolean);
  } else {
    taskIds = [];
  }
  if (taskIds.length === 0) {
