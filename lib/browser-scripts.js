...
(() => {
  ...
  const normalizeTaskUrl = (doc, taskId) => {
    const url = new URL(doc.location?.href || location.href, location.href);
    url.searchParams.delete('sid');
    if (taskId) url.searchParams.set('taskInstId', taskId);
    return url.href;
  };
  ...
  for (const doc of documents) {
    ...
    return taskRows.slice(0, limit).map((row, index) => {
      ...
      return {
        ...
        status: 'pending',
        url: normalizeTaskUrl(doc, taskId),
      };
    });
  }
