...
export function todosScript(limit, filter = {}) {
  return `
(async () => {
  const limit = ${Number(limit)};
  const filterType = ${JSON.stringify(filter.type || '')};
  const filterStatus = ${JSON.stringify(filter.status || 'pending')};
  const clean = (value) => String(value || '').replace(/\\s+/g, ' ').trim();
  ...
  for (const frame of Array.from(document.querySelectorAll('iframe'))) {
    try {
      ...
      for (const child of Array.from(frame.contentDocument?.querySelectorAll('iframe') || [])) {
        if (child.contentDocument) documents.push(child.contentDocument);
      }
    } catch (error) {
      // Ignore cross-origin frames; BONC OA workbench frames are expected to be same-origin.
    }
  }

  const extractRows = (doc, status) => {
    const selector = status === 'done'
      ? '#tableData tr.cdata, table#tableData tr[class*=cdata], table#tableData tr[class*=done], #tableData tr[class*=processed]'
      : '#tableData tr.tdata, table#tableData tr[class*=tdata]';
    const taskRows = Array.from(doc.querySelectorAll(selector));
    if (!taskRows.length) return [];
    return taskRows.map((row, index) => {
      const cells = Array.from(row.children).map((cell) => clean(cell.innerText || cell.textContent));
      const taskId = String(row.id || '').replace(/^tr-/, '') || row.querySelector('input[type=checkbox]')?.value || String(index + 1);
      if (status === 'done') {
        return {
          taskId,
          title: cells[3] || clean(row.querySelector('.navBox')?.getAttribute('title')) || clean(row.innerText).slice(0, 80),
          type: cells[4] || '',
          receivedAt: cells[5] || '',
          result: cells[6] || '',
          processedAt: cells[7] || '',
      ...
      const receivedTitle = row.children[5]?.querySelector('[title]')?.getAttribute('title') || cells[5] || '';
      return {
        taskId,
        title: cells[3] || clean(row.querySelector('.navBox')?.getAttribute('title')) || clean(row.innerText).slice(0, 80),
        type: cells[4] || '',
        receivedAt: receivedTitle,
      };
    });
  };
  ...
  const findNextButton = (doc) => {
    ...
    return buttons.find((btn) => {
      const text = clean(btn.innerText || btn.textContent || btn.value || '');
      ...
      return !disabled && (
        ...
        /[>》]\\s*$/.test(text) || id.includes('next') ||
        onclick.includes('nextPage') || onclick.includes('next') ||
  ...
  while (rows.length < limit && hasMorePages) {
    const currentDocs = [pageDoc];
    for (const frame of Array.from(pageDoc.querySelectorAll('iframe'))) {
      try {
        if (frame.contentDocument) currentDocs.push(frame.contentDocument);
      } catch (error) { /* cross-origin */ }
    }

    for (const doc of currentDocs) {
      const extracted = extractRows(doc, filterStatus);
      for (const row of extracted) {
        if (filterType && !row.type.includes(filterType)) continue;
        const key = row.taskId + row.title;
        if (!seen.has(key)) {
          seen.add(key);
          rows.push(row);
        }
    ...
    const nextBtn = findNextButton(pageDoc);
    if (!nextBtn) {
      hasMorePages = false;
      break;
    }
    ...
    const dataFrame = newPageDoc.querySelector('iframe#dataFrame');
    if (dataFrame && dataFrame.contentDocument) {
      pageDoc = dataFrame.contentDocument;
    } else {
      pageDoc = newPageDoc;
    }

    const newNextBtn = findNextButton(pageDoc);
    if (newNextBtn && newNextBtn === nextBtn) {
      hasMorePages = false;
    }
  }
  ...
  const nodes = documents.flatMap((doc) => Array.from(doc.querySelectorAll('tr, [role=row], li, .todo, .task, .workflow, .list-item, [class*=todo], [class*=task], [class*=work], [class*=flow]')));
  for (const node of nodes) {
    ...
    const url = link ? new URL(link.getAttribute('href'), node.ownerDocument?.location?.href || location.href).href : (node.ownerDocument?.location?.href || location.href);
    const idMatch = (url + ' ' + text).match(/(?:task|todo|work|flow|process|id)[_=/-]([A-Za-z0-9-]+)/i) || text.match(/\\b[A-Z0-9]{6,}\\b/);
    const taskId = idMatch ? idMatch[1] || idMatch[0] : String(fallback.length + 1);
    ...
    fallback.push({
      ...
      type: cells.find((v) => /流程|审批|报销|请假/.test(v)) || '',
...
export function openWorkbenchScript() {
  return `
(() => {
  const clean = (value) => String(value || '').replace(/\\s+/g, ' ').trim();
  const visible = (node) => {
  ...
  const workbench = frames.find((frame) => /workbench_main_page/.test(frame.src || ''));
  if (workbench) {
    let dataFrame = null;
    try {
      dataFrame = workbench.contentDocument?.querySelector('#dataFrame');
    } catch (error) {
      dataFrame = null;
    }
    return {
      ok: true,
      opened: false,
      reason: dataFrame ? 'workbench-ready' : 'workbench-frame-ready',
      url: location.href,
      workbenchUrl: workbench.src || '',
      dataUrl: dataFrame?.src || '',
    };
  }

  const nodes = Array.from(document.querySelectorAll('#obj_c867594519be463faadfe4e4a9aa25d1, [id^=obj_], li, div, a, span')).filter(visible);
  const target = nodes.find((node) => node.id === 'obj_c867594519be463faadfe4e4a9aa25d1')
    || nodes.find((node) => clean(node.innerText || node.textContent) === '工作台')?.closest('[id^=obj_], li, div, a')
    || nodes.find((node) => /工作台/.test(clean(node.innerText || node.textContent)))?.closest('[id^=obj_], li, div, a');
  if (!target || !visible(target)) {
    return { ok: false, opened: false, reason: 'workbench-entry-not-found', url: location.href };
  }
  target.click();
  return { ok: true, opened: true, reason: 'clicked-workbench', url: location.href };
})()
`;
}
